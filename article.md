...
>
> Когда вы добавляете пятнадцатого агента в команду, простые решения перестают работать. Эта статья — о том, как превратить хаос мультиагентной координации в воспроизводимый процесс, не изобретая велосипед.

---

## 1. Проблема

SDLC формализован в документации процесса + bash-скрипты (CLI-скрипты управления бордом, скрипт верификации, gatekeeper в CLI-скриптах управления бордом). Это самодельное решение с рядом проблем:

- **Gatekeeper на bash** — скрипт-«сторож», который проверял, можно ли перевести задачу на следующий этап. Был написан на bash: цепочка `if/then` с вызовами `gh api` и проверкой полей. Bash прекрасен для склеивания команд в пайплайн, но не для бизнес-логики: нет типизации (строка вместо числа — и скрипт молча пропускает ошибку), сложно обрабатывать нестандартные ситуации, а каждое новое правило — это ещё один вложенный `if`. Когда процесс меняется, bash-скрипт ломается — и никто не замечает, пока не поздно.

- **Верификаторы на grep/python** — проверки качества, которые искали текстовые признаки вместо проверки архитектурных контрактов. Пример: верификатор grep'ал код в поисках строки `TODO` — и радостно отмечал проблему, если находил. Но если вся архитектура нарушена (модуль А зависит от модуля Б, который зависит от модуля А), верификатор ничего не замечал: строки-то правильные, а круговая зависимость grep'ом не ловится. Проблема не в самом grep — проблема в подходе: проверяют текст, а не контракты.
...
- **Фантомный Done** — задача помечена как выполненная, но артефакта нет. Почему? Системе неоткуда проверить, что артефакт действительно существует и соответствует критериям приёмки. Верификатор не проверял контракты, gatekeeper пропускал — и вот задача в «Done», а результата нет. Читатель бэклога видит «Done», а заказчик — дыру.

Мультиагентный хаос — это не проблема людей и не проблема моделей. Это проблема **процесса**. Пятнадцать агентов без формализованного процесса — как пятнадцать программистов без Agile: каждый делает что-то полезное, но вместе они делают хаос. Причём агенты применяются не только для разработки — продажи, саппорт, аналитика, маркетинг: любой повторяющийся процесс с фазами и артефактами подвержен той же проблеме. SDLC (Software Development Life Cycle) — это не только про код. Это общая идея: процесс состоит из фаз, переход между фазами требует артефактов, а артефакты нужно проверять. Фазы продаж (лид → квалификация → предложение → сделка) отличаются от фаз разработки (requirements → design → implementation → QA → deploy), но принцип тот же: нет артефакта — нет перехода. Индустрия уже стандартизировала протоколы для формализации таких процессов. Попытка написать HTTP своими руками избыточна.
